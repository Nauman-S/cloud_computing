  ...
  return (
    <div className="container mt-4">
      ...
      <Table striped bordered hover responsive>
        ...
        <tbody>
          {data.map((item) => {
            // Parse the JSON string from applicationSummary
            let summary = {};
            try {
            ...
            return (
              <tr key={item._id}>
                <td>{item.applicationNum || summary.applicationNum || "—"}</td>
                <td>{summary.applicationType || "—"}</td>
                <td>{summary.TitleOfInvention || "—"}</td>
                <td>{item.lodgementDate || summary.lodgementDate || "—"}</td>
              </tr>
            );
          })}
        </tbody>
